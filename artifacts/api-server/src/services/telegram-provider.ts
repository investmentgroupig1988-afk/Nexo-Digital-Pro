import { signalActiveMessage, type NotificationProvider, type NotificationTimeframe } from "./notification-provider";
...
export class TelegramProvider implements NotificationProvider {
  ...
  async sendSignalActive(publicUrl: string, timeframe: NotificationTimeframe): Promise<void> {
    const response = await this.request(`https://api.telegram.org/bot${this.token}/sendMessage`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ chat_id: this.chatId, text: signalActiveMessage(publicUrl, timeframe), disable_web_page_preview: false }),
