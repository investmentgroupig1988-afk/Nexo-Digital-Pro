...
test("public notification contains only availability and the configured link", () => {
  const message = signalActiveMessage("https://staging.nexodigitalpro.lat/");
  assert.match(message, /SEÑAL ACTIVA/);
  assert.match(message, /https:\/\/staging\.nexodigitalpro\.lat\//);
...
test("telegram sends no token or signal details in the JSON body", async () => {
  ...
  await new TelegramProvider("secret-token", "@channel", request).sendSignalActive("https://staging.nexodigitalpro.lat/");
...
test("telegram errors do not include upstream response bodies", async () => {
  ...
  await assert.rejects(() => new TelegramProvider("secret-token", "@channel", request).sendSignalActive("https://staging.nexodigitalpro.lat/"), /HTTP 500/);
});
