...
type Props = { initialTimeframe?: MarketTimeframe; onAccount?: () => void; onAdmin?: () => void; onLogout?: () => void };
type HistoryScope = MarketTimeframe | "all";

export function MarketDashboard({ initialTimeframe = "15m", onAccount, onAdmin, onLogout }: Props) {
  const [timeframe, setTimeframe] = useState<MarketTimeframe>(initialTimeframe);
  const [historyTimeframe, setHistoryTimeframe] = useState<HistoryScope>(initialTimeframe);
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#070812] text-slate-100">
    <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 lg:px-8">
      ...
      <footer className="mt-8 border-t border-white/8 py-6 text-center text-xs text-slate-500">Las señales informan movimiento técnico porcentual. No representan ganancias personales ni constituyen asesoramiento financiero. Sistema {health.isSuccess ? "conectado" : health.isError ? "no disponible" : "en verificación"}.</footer>
    </div>
  </main>;
}
...
function directionLabel(value: CommercialSignal["direction"]) { return value === "LONG" ? "ALCISTA" : "BAJISTA"; }
