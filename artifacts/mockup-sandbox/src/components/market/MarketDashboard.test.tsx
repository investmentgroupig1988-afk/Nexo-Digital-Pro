...
describe("commercial signal dashboard", () => {
  ...
  it("shows entry, SL, TP and R:R for a real active signal", async () => { api.getSignalDashboard.mockResolvedValue(active); renderDashboard(); expect(await screen.findByText("ALCISTA")).toBeTruthy(); expect(screen.queryByText("LONG")).toBeNull(); expect(screen.getByText("Entrada")).toBeTruthy(); expect(screen.getByText("Stop loss")).toBeTruthy(); expect(screen.getByText("Take profit")).toBeTruthy(); expect(screen.getByText("1:1,5")).toBeTruthy(); });
